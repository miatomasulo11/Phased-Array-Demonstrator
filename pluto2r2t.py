...
rx_gain0 = 10           # these gains ignored in slow-attack, can be used in manual
rx_gain1 = 10
tx_lo = rx_lo
tx_gain0 = -10
tx_gain1 = -10
...
i0 = np.cos(2 * np.pi * t * fc0 + 45) * 2 ** 14 
q0 = np.sin(2 * np.pi * t * fc0 + 45) * 2 ** 14
i1 = np.cos(2 * np.pi * t * fc1) * 2 ** 14
q1 = np.sin(2 * np.pi * t * fc1) * 2 ** 14
iq0 = i0 + 1j * q0
iq1 = i1 + 1j * q1
sdr.tx([iq0, iq1])   # Send Tx data.w
